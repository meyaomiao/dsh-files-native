  ...
  const upload = webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        const file2 = await writeUpload(cwd, fileName, bytes);
        file2.mediaType = q.get("type") || file2.mediaType;
        const list = pending.get(sessionId) ?? [];
        list.push(file2);
        pending.set(sessionId, list);
        json(res, 200, { ok: true, file: file2 });
      } catch (error) {
  ...
  const file = webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        const name2 = relPath.replace(/^.*[/\\]/, "");
        res.writeHead(200, {
          "content-type": types[ext] ?? "application/octet-stream",
          "content-length": String(bytes.length),
          "content-disposition": `inline; filename*=UTF-8''${encodeURIComponent(name2)}`
        });
        res.end(bytes);
      } catch {
  ...
  ctx.on("agent/pre-step", async (...args) => {
    ...
    const notice = {
      ...
      source: {
        ...
        plugin: "file-native",
        form: "notice",
        summary: `\u{1F4CE} \u9644\u4EF6 ${deliver.length} \u4E2A\u6587\u4EF6`
      }
    };
    return {
      ...decision,
