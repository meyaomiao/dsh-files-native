...
function extractPasteText(raw, files) {
  const text = raw.replace(/\uFFFC/g, "").trim();
  if (text === "") return "";
  const names = files.map((file) => file.name.replace(/^.*[/\\]/, "")).filter((name2) => name2.length >= 4);
  if (names.length === 0) return text;
  const tokens = text.split(/\s+/).filter(Boolean);
  const referenced = (token) => names.some((name2) => token.toLowerCase().includes(name2.toLowerCase()));
...
function pasteAccompanyingText(text) {
...
function FileCard({ file, onOpen }) {
  return (0, import_react6.createElement)(
    ...
    (0, import_react6.createElement)("span", { className: "fr-ext" }, extOf(file.name)),
    (0, import_react6.createElement)(
      "span",
      { className: "fr-copy" },
      (0, import_react6.createElement)("span", { className: "fr-name" }, file.name),
      file.size > 0 ? (0, import_react6.createElement)("span", { className: "fr-meta" }, formatSize(file.size)) : null
    )
  );
}
...
function MessageCards(props) {
  ensureStyles();
  const [hosted, setHosted] = (0, import_react6.useState)([]);
  const sessionKey = (0, import_react6.useRef)("");
  const lastCount = (0, import_react6.useRef)(-1);
  const cachedFiles = (0, import_react6.useRef)(null);
  ...
  return (0, import_react6.createElement)(
    ...
    ...hosted.map((item, i) => (0, import_react_dom3.createPortal)(
      (0, import_react6.createElement)(
        ...
        ...item.files.map((file) => (0, import_react6.createElement)(FileCard, { key: file.relPath, file, onOpen: props.onOpen }))
      ),
      item.host,
      `file-native-cards-${i}`
    ))
...
function RailSlot(props) {
  ...
  rememberSessionId(sessionId);
  const tokenRef = (0, import_react7.useRef)(0);
  ...
  (0, import_react7.useLayoutEffect)(() => () => clearLiveOwner(tokenRef.current), []);
  return null;
}
var chatCardOpener;
function PickerSlot(props) {
  ...
  return (0, import_react7.createElement)(
    "span",
    null,
    (0, import_react7.createElement)(PaperclipButton, { sessionId }),
    (0, import_react7.createElement)(MessageCards, { sessionId, onOpen: (file) => chatCardOpener?.(sessionId || currentSessionId(), file.relPath, file.name) })
  );
}
